	...
	try {
		...
		do {
			...
			case LOOKING_FOR_BLOCK: {
				...
				if (fileSource != NULL && fileSource->fileSize() == fileSource->getFilePosition())
				{
					restartPlayback();
				}
...
void MatroskaFileParser::restartPlayback()
{
	ByteStreamFileSource* fileSource = (ByteStreamFileSource*)fInputSource; // we know it's a "ByteStreamFileSource"
	if (fileSource != NULL)
	{
		fileSource->seekFileBegin();
		resetStateAfterSeeking();
		fPresentationTimeOffset += fLastPresentationTime;
	}
}